    ...
    <article class="pipeline-card" data-id="${item.id}" draggable="true">
      ...
      <div class="pipeline-card-footer">
        ...
        <div class="pipeline-contact-row">
          <button type="button" class="btn-whatsapp-luxury btn-wa-concierge" data-id="${item.id}" title="Centro de Mensajes de WhatsApp">
  ...
  Swal.fire({
    ...
    html: `
      ...
      <div style="display: flex; flex-direction: column; gap: 8px;">
        ...
        <button id="swalOpenWhatsApp" class="btn btn-secondary" style="font-size: 0.85rem; text-align: left; padding: 10px; color: #25d366; border-color: rgba(37,211,102,0.3); display: flex; align-items: center; gap: 8px;">
          💬 Abrir Centro de Mensajes WhatsApp
        </button>
    ...
    didOpen: () => {
      ...
      document.getElementById("swalOpenWhatsApp")?.addEventListener("click", () => {
        Swal.close();
        openWhatsAppModal({
          id: item.id,
          clientName: item.nombre,
          phone: item.telefono,
          eventType: item.evento || item.tipo_evento,
          guestCount: item.personas,
          location: item.locacion,
          eventDate: item.fechaEvento || item.fecha_evento,
          totalAmount: Number(item.totalEstimado || item.total || 0),
          origin: "cotizacion"
        });
      });
...
function asignarEventosAcciones() {
  // Botones directos de WhatsApp Concierge
  document.querySelectorAll(".btn-wa-concierge").forEach(btn => {
