...
```bash
git clone https://github.com/torvi30/banquetes-almar.git
cd "banquetes almar"
```
...
The build scripts will automatically generate `frontend/js/firebase/env.js` from your `.env`.
...
|---|---|---|
| `dev` | `npm run dev` | Generates client env, runs Tailwind watch and serves `frontend` on port 5000 |
| `generate:env` | `npm run generate:env` | Generates `frontend/js/firebase/env.js` from root `.env` |
...
| `db:seed` | `npm run db:seed` | Seeds default packages and rental inventory into Firestore from `seeds/` |
| `db:migrate` | `npm run db:migrate` | Migrates structure and initial data to Firestore |
